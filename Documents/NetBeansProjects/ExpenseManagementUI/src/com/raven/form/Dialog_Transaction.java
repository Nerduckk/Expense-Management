...
private void onSave() {
    ...
    // ==========================
    // CHẶN TIÊU QUÁ SỐ DƯ VÍ
    // ==========================
    if (!isIncome) {  // chỉ kiểm tra với giao dịch chi
        BigDecimal newAmount = amount;
        BigDecimal projectedBalance = account.getBalance() == null
                ? BigDecimal.ZERO
                : account.getBalance();

        // Nếu đang SỬA giao dịch và vẫn dùng cùng 1 ví,
        // thì cộng/trừ lại ảnh hưởng của giao dịch cũ vào projectedBalance
        if (editingTransaction != null) {
            Account oldAcc = editingTransaction.getSourceAccount();
            if (oldAcc != null
                    && oldAcc.getId() != null
                    && account.getId() != null
                    && oldAcc.getId().equals(account.getId())) {

                BigDecimal oldAmt = editingTransaction.getAmount() == null
                        ? BigDecimal.ZERO
                        : editingTransaction.getAmount().abs();

                if (editingTransaction.isIncome()) {
                    // Giao dịch cũ là thu: lúc undo sẽ TRỪ đi
                    projectedBalance = projectedBalance.subtract(oldAmt);
                } else if (editingTransaction.isExpense()) {
                    // Giao dịch cũ là chi: lúc undo sẽ CỘNG lại
                    projectedBalance = projectedBalance.add(oldAmt);
                }
            }
        }

        if (projectedBalance.compareTo(newAmount) < 0) {
            JOptionPane.showMessageDialog(this,
                    "Số dư ví không đủ để chi " + newAmount.toPlainString()
                    + "\nSố dư khả dụng: " + projectedBalance.toPlainString(),
                    "Không đủ tiền",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    if (editingTransaction == null) {
        ...
        // Hoàn tác giao dịch cũ + xóa khỏi list
